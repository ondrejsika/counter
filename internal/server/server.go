...
func BaseServer(
	...
	counterAPIHandler := func(countFunc func() (int, error)) http.HandlerFunc {
...
func Server(dontRunMigrations bool, versionOverride string) {
	...
	if backend == "redis" {
		...
		runMigrationsFunc = func() error { return backend_redis.RunFakeMigrations(redisHost, redisPassword, hostname, redisSchema) }
